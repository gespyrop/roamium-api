...
ssh -i /tmp/deploy_rsa george@roamium.software cd $ROAMIUM_DIRECTORY && \
    ...
    docker-compose --env-file $ROAMIUM_DIRECTORY/production.env down && \
    docker-compose --env-file $ROAMIUM_DIRECTORY/production.env pull && \
    docker-compose --env-file $ROAMIUM_DIRECTORY/production.env up -d
