...
scp -i /tmp/deploy_rsa -r env/production.env george@roamium.software:$ROAMIUM_DIRECTORY
...
ssh -i /tmp/deploy_rsa george@roamium.software cd $ROAMIUM_DIRECTORY && \
    ...
    docker-compose --env-file $ROAMIUM_DIRECTORY/env/production.env down && \
    docker-compose --env-file $ROAMIUM_DIRECTORY/env/production.env pull && \
    docker-compose --env-file $ROAMIUM_DIRECTORY/env/production.env up -d
